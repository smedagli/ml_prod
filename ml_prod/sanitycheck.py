...
def run_sanity_check(test_dir):
    ...
    if not test_functions_for_post:
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            text = "One or more of your test cases for POST() do not seem to be testing the contents of the response.\n"
            print(
                FAIL_COLOR + text)
        ...
        if len(test_functions_for_post) >= 2 and COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            text = "You do not seem to have TWO separate test cases, " \
                   "one for each possible prediction that your model can make."
            print(FAIL_COLOR + text)

    SANITY_TEST_PASSING = SANITY_TEST_PASSING and \
        TEST_FOR_GET_METHOD_RESPONSE_CODE and TEST_FOR_GET_METHOD_RESPONSE_BODY and \
        TEST_FOR_POST_METHOD_RESPONSE_CODE and TEST_FOR_POST_METHOD_RESPONSE_BODY and \
